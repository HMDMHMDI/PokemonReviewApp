...
public class PokemonController : Controller
{
    ...
    [ProducesResponseType(400)]
    public IActionResult CreatePokemon([FromBody] PokemonDto pokemonCreate , [FromQuery] int catId , [FromQuery] int ownerId)
    {
        if (pokemonCreate == null) return BadRequest(ModelState);
        var pokemons = _pokemonRepository.GetPokemons()
            .Where(e => e.Name.Trim().ToUpper() == pokemonCreate.Name.TrimEnd().ToUpper())
            .FirstOrDefault();
        if (pokemons != null)
        {
        ...
        [FromBody] PokemonDto updatedPokemon)
    {
        ...
        if (!_pokemonRepository.UpdatePokemon(pokemonMap , ownerId,catId))
        {
    ...
    public IActionResult DeletePokemon(int pokeId)
    {
        ...
        {
            ModelState.AddModelError("" , "something wrong while deleting reviews");
        }
